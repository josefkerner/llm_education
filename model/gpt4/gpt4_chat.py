...
class GPT4_turbo_model(Model):
    def __init__(self, cfg: Dict):
        self.cfg = cfg
        self.prompt_template = cfg['prompt_template'] if 'prompt_template' in cfg else None
        self.MAX_CHARS = 8000

    def get_gpt_chat_prompts(self, base_prompts: List[str]):
        prompts = []
        for base_prompt in base_prompts:
            prompt = [
                {'role':'system','content':'You are a helpfull data analytics assistant'},
    ...
    def split_chunks(prompts, max_tokens: 5000):
        ...
        for i, prompt in enumerate(prompts):
            prompt_token_len = GPT4_turbo_model.estimate_tokens(prompt)
    ...
    def generate(self, prompts: List[List[Dict]],temp: int = 0.0, functions: List[Dict] = None):
        ...
        chunks, all_ids = GPT4_turbo_model.split_chunks(prompts,
