...
class ProductCatalogChatbot:
    ...
    def check_context_needed(self, chat_history: List[str]):
        ...
        if "PRODUCT_RECOMMENDED" in chat_history:
            return False
        prompt = f"""
        Verify if the given chat history is specific enough to recommend a product from a product database.
        chat history should contain specific product features.
        Answer with only 'yes' or 'no'.
        Chat history: {chat_history_str}
        Answer:
        """
        prompt_dict = [
            {"role": "system", "content": "You are expert in product catalog domain."},
    ...
    def recommend_product(self, chat_history: List[str]):
        ...
        if "PRODUCT_RECOMMENDED" in chat_history:
            product_features = f"""
            Answer a product related question from a product database based on given chat history.
            Always respond in the same language as the user requests.
            Never use familiar language.
            Chat history: {chat_history_str}
            """
            ...
            Example: --------------------
            'User: Hele chtěl bych bundu', 'Bot: Jaké konkrétní vlastnosti byste chtěl, aby tato bunda měla?', 'User: noo asi dobrej vodní sloupec', 'Bot: Jaký by měl být vodní sloupec této bundy?', 'User: aspoň 5000 mm'
            Extracted product features: jacket, water column of at least 5000 mm
            -----------------------------
            Chat history: {chat_history_str}
    ...
    def converse(self):
        ...
        while True:
            ...
            response.chat_history.append(f"Bot: PRODUCT_RECOMMENDED")
...
if __name__ == "__main__":
    ...
    chatbot.add_docs()
    message = f"""
    Welcome to the product catalog chatbot.
    I will help you to find the right Jacket you are looking for.
